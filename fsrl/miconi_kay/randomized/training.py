"""Fixed-budget training for the randomized reward-only source candidate."""
...
from fsrl.miconi_kay.randomized.protocol import (
    RANDOMIZED_SOURCE_TRAINING_CONFIGURATION,
    randomized_source_evaluation_plan,
    randomized_source_training_plan,
)
...
class RandomizedFineTuneConfig:
    episodes: int = 1000
...
@dataclass(frozen=True)
class RandomizedFineTuneSummary:
    episode_index: int
    loss_value: float
    item_count: int
    bridge_trials: int
...
def randomized_tree_sha256() -> str:
...
def _observed_training_configuration(
    ...
    fine_tune: RandomizedFineTuneConfig,
) -> dict[str, object]:
    return {
...
def register_randomized_protocol(
    ...
    *,
    ...
    if 8 in curriculum_config.item_counts:
        raise ValueError("formal challenge item count 8 cannot enter training")
    if curriculum_config.maximum_bridge_trials >= 4:
        raise ValueError("formal four-bridge duration cannot enter training")
    registration = {
        ...
        "claim_ceiling": (
            "randomized M&K-derived source candidate; not source parity, Liu "
        ...
        "registered_training_plan": randomized_source_training_plan(),
        "observed_training_configuration": observed,
        "is_formal_training_configuration": (
            observed == RANDOMIZED_SOURCE_TRAINING_CONFIGURATION
        ),
        "evaluation_plan": randomized_source_evaluation_plan(),
...
def _save_checkpoint(
    ...
    *,
    ...
    curriculum_config: RandomizedLinkedCurriculumConfig,
    fine_tune_config: RandomizedFineTuneConfig,
    history: list[RandomizedFineTuneSummary],
    ...
    manifest = {
        "claim_status": (
            "randomized M&K-derived source-only candidate; not source parity, "
...
def train_randomized_finetune(
    *,
    ...
    list[RandomizedFineTuneSummary],
    Path,
]:
    ...
    set_source_seed(fine_tune_config.seed)
    registration_path = register_randomized_protocol(
        output_dir,
    ...
    for episode_index in range(fine_tune_config.episodes):
        ...
        stats = run_randomized_linked_episode(
        ...
        optimizer.step()
        summary = RandomizedFineTuneSummary(
        ...
        if (
            ...
            print(
                "randomized-source-v2 "
                f"episode={episode_index + 1}/{fine_tune_config.episodes} "
                f"items={summary.item_count} bridge={summary.bridge_trials} "
                f"loss={summary.loss_value:.6f} "
                f"query_acc={summary.query_accuracy:.4f} "
                f"query_p={summary.mean_correct_choice_probability:.4f}"
            )
    output_checkpoint = _save_checkpoint(
        model,
        output_dir,
        base_checkpoint=checkpoint_path,
        source_config=source_config,
