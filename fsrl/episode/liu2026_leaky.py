"""Sign-only Liu/meta-training episode for the leaky accumulator baseline."""
...
class LeakyAccumulatorEpisodeResult:
    loss: torch.Tensor
    logits: torch.Tensor
...
def run_leaky_accumulator_episode(
    config,
    net: LeakyRankAccumulator,
    *,
    rng=None,
    ...
    """Run a differentiable episode with query-time state held fixed."""

    if not isinstance(net, LeakyRankAccumulator):
        raise TypeError("leaky accumulator runner requires LeakyRankAccumulator")
    if rng is None:
        rng = np.random
    ...
    if task_regime == "liu_eval":
        ...
        raise ValueError(f"unknown leaky task_regime: {task_regime!r}")
    ...
    for trial_index in range(num_support_trials):
        ...
        state = net.update_support(state, left, right, sign)
    ...
    return LeakyAccumulatorEpisodeResult(
        loss=loss,
        logits=logits,
        targets=targets,
        accuracy=accuracy,
...
def run_leaky_accumulator_eval_episode(
    config,
    net: LeakyRankAccumulator,
    *,
    rng=None,
    ...
    if rng is None:
        rng = np.random
    with torch.no_grad():
        result = run_leaky_accumulator_episode(
    ...
    return symbolic_logits_to_episode_record(
        ...
        rng=rng,
        provenance={
            "runner": "leaky_rank_accumulator_meta_sign_v1",
