"""Minimal winner-biased item learning plus direct-pair episodic memory.

This model is the observable-equivalence reduction of AADM-v2 after entropy
adaptation could not be recovered from binary choices.  It retains no entropy
temperature or asymmetry selector: feedback updates the winner only.
"""
...
class WinnerBiasedDualMemoryConfig:
    max_items: int = 8
    cue_dim: int = 15
...
@dataclass
class WinnerBiasedDualMemoryState:
...
@dataclass(frozen=True)
class WinnerBiasedIntervention:
...
@dataclass
class WinnerBiasedEncodingTrace:
...
@dataclass
class WinnerBiasedConsolidationTrace:
    event: str
...
class WinnerBiasedDualMemory(nn.Module):
    """Winner-only value cache with an independently lesionable pair trace."""

    _LEGAL_EVENTS = frozenset(("phase_transition", "visible_boundary"))

    def __init__(self, config: WinnerBiasedDualMemoryConfig):
        super().__init__()
        self.config = config
    ...
    def initial_state(self, batch_size: int) -> WinnerBiasedDualMemoryState:
        if batch_size <= 0:
        ...
        return WinnerBiasedDualMemoryState(
    ...
    def _validate_pair_inputs(self, state, cue_features, left_cue, right_cue) -> None:
    ...
    def observe_relation(
        self,
        state: WinnerBiasedDualMemoryState,
        ...
        *,
    ...
    ) -> tuple[WinnerBiasedDualMemoryState, WinnerBiasedEncodingTrace]:
        if intervention is None:
            intervention = WinnerBiasedIntervention()
        ...
        correct_probability = torch.sigmoid(winner_value - loser_value)

        rate_noise = torch.zeros_like(sign)
        if stochastic:
        ...
        winner_rate = torch.sigmoid(self.base_learning_rate_logit + rate_noise)
        loser_rate = torch.zeros_like(winner_rate)
        ...
        loser_delta = torch.zeros_like(winner_delta)
        ...
        if getattr(intervention, "local_update_enabled", True):
            ...
            local_precision = (
                local_precision + write_strength[:, None, None] * undirected_edge
            )

        next_state = WinnerBiasedDualMemoryState(
            item_values=item_values,
            local_evidence=local_evidence,
            local_precision=local_precision,
            update_count=state.update_count + 1,
        )
        return next_state, WinnerBiasedEncodingTrace(
    ...
    def consolidate(
        ...
        *,
        stochastic: bool = True,
        generator: torch.Generator | None = None,
    ) -> tuple[WinnerBiasedDualMemoryState, WinnerBiasedConsolidationTrace]:
        ...
        return state, WinnerBiasedConsolidationTrace(
            event=str(value),
            proposed_scores=state.item_values,
            score_change=torch.zeros_like(state.item_values),
    ...
    def query_pair(
        ...
        *,
        ...
        return WinnerBiasedQueryTrace(
            logits=torch.stack(
                (torch.log(probability), torch.log1p(-probability)), dim=1
            ),
...
def load_aadm_fixed_winner_state(
    model: WinnerBiasedDualMemory, checkpoint: dict
) -> None:
    if checkpoint.get("candidate") != "AdaptiveAsymmetricDualMemory-v2-stationary-meta":
        raise ValueError("checkpoint is not from the frozen AADM-v2 budget")
    if checkpoint.get("regime") != "fixed_winner_only":
        raise ValueError("only the fixed_winner_only regime can be extracted")
    source = checkpoint["state_dict"]
    if float(source["entropy_temperature_log"]) != 0.0:
        raise ValueError("winner-only checkpoint unexpectedly changed unused entropy")
    target_keys = set(model.state_dict())
    extracted = {key: value for key, value in source.items() if key in target_keys}
    if set(extracted) != target_keys:
        raise ValueError("checkpoint is missing winner-biased parameters")
